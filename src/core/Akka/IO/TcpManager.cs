            ...
            //    if (completed != null)
                ...
                if (c.Options.Any(r => r is Inet.SO.TlsConnectionOption))
                {
                    Context.ActorOf(Props.Create<TlsOutgoingConnection>(() =>
                        new TlsOutgoingConnection(_tcp, commander, c)).WithDispatcherIfNeeded(c.Options).WithDeploy(Deploy.Local), NextTcpOutgoingConnectionName());
                }
                else
                {
                    Context.ActorOf(Props.Create<TcpOutgoingConnection>(_tcp, commander, c)
                        .WithDispatcherIfNeeded(c.Options).WithDeploy(Deploy.Local), NextTcpOutgoingConnectionName());
                }
                ...
                case Bind b:
                {
                    ...
                    if (b.Options.Any(r => r is Inet.SO.TlsConnectionOption))
                    {
                        Context.ActorOf(Props.Create<TlsListener>(() =>
                            new TlsListener(_tcp, commander, b)).WithDeploy(Deploy.Local), NextTcpListenerName());
                    }
                    else
                    {

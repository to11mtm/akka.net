...
public abstract class ActorPath : IEquatable<ActorPath>, IComparable<ActorPath>, ISurrogated
{
    ...
    public sealed class Surrogate : ISurrogate, IEquatable<Surrogate>, IEquatable<ActorPath>
    {
        ...
        /// <exception cref="SerializationException">Thrown when surrogate <see cref="Path"/> could not be parsed into a valid <see cref="ActorPath"/></exception>
        public ISurrogated FromSurrogate(ActorSystem system)
        {
            if (!TryParse(Path, out var path))
                throw new SerializationException("Failed to parse actor path surrogate");
            return path;
        }
    ...
    {
        // guard for possible null parentPath
        Parent = parentPath ?? throw new ArgumentNullException(nameof(parentPath));
    ...
    private static int InternalCompareTo(ActorPath? left, ActorPath? right)
    {
        while (true)
        {
            ...
            if (left._depth == 0) return left.CompareTo(right);

            if (right._depth == 0) return -right.CompareTo(left);
    ...
    public ActorPath WithUid(long uid)
    {
        ...
        // Parent should never be null here, unless we have a malformed/corrupted ActorPath
        if (Parent is null)
            throw new InvalidOperationException("ActorPath.Parent should never be null when Depth is greater than 0");
    ...
    /// </summary>
    /// <param name="path">The string actor path to be parsed</param>
    /// <param name="actorPath">When this method returns, contains the parsed <see cref="ActorPath"/> value is the operation succeeded, <c>null</c> if it failed</param>
    /// <returns><c>true</c> if <paramref name="path"/> can be parsed into a valid actor path</returns>
    public static bool TryParse(string path, [NotNullWhen(true)] out ActorPath? actorPath)
    {
    ...
    /// </summary>
    /// <param name="basePath">The base path, normally a root path</param>
    /// <param name="absoluteUri">The path components to be appended to the <paramref name="basePath"/></param>
    /// <param name="actorPath">When this method returns, contains the concatenated <see cref="ActorPath"/> value of <paramref name="basePath"/> and <paramref name="absoluteUri"/></param>
    ...
    /// <returns><c>true</c> if the <see cref="Address"/> could be parsed, <c>false</c> otherwise.</returns>
    public static bool TryParseAddress(string path, [NotNullWhen(true)] out Address? address, out ReadOnlySpan<char> absoluteUri)
    {
        address = null;
    ...
    private string Join(ReadOnlySpan<char> prefix, long? uid = null)
    {
        ...
        else
        {
            ...
            var buffer = totalLength < 1024 ? stackalloc char[totalLength] : new char[totalLength];
            prefix.CopyTo(buffer);
    ...
    public override bool Equals(object? obj)
    {
        return obj is ActorPath path && Equals(path);
    }
    ...
    private string ToStringWithAddress(Address address, bool includeUid)
    {
        ...
        if (Address is { Host: not null, Port: not null })

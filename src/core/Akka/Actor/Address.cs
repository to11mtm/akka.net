...
public sealed class Address : IEquatable<Address>, IComparable<Address>, IComparable, ISurrogated, ICloneable
{
    ...
    public static bool TryParse(string path, [NotNullWhen(true)] out Address? address)
    {
    ...
    /// <param name="path">The span of address to parse</param>
    /// <param name="address">If <c>true</c>, the parsed <see cref="Address"/>. Otherwise <c>null</c>.</param>
    /// <param name="absolutUri">If <c>true</c>, the absolut uri of the path. Otherwise <c>null</c>.</param>
    /// <returns><c>true</c> if the <see cref="Address"/> could be parsed, <c>false</c> otherwise.</returns>
    public static bool TryParse(string path, [NotNullWhen(true)] out Address? address, [NotNullWhen(true)] out string? absolutUri)
    {
    ...
    /// <param name="address">If <c>true</c>, the parsed <see cref="Address"/>. Otherwise <c>null</c>.</param>
    /// <returns><c>true</c> if the <see cref="Address"/> could be parsed, <c>false</c> otherwise.</returns>
    public static bool TryParse(ReadOnlySpan<char> span, [NotNullWhen(true)] out Address? address)
    {
    ...
    /// <returns><c>true</c> if the <see cref="Address"/> could be parsed, <c>false</c> otherwise.</returns>
    public static bool TryParse(ReadOnlySpan<char> span, [NotNullWhen(true)] out Address? address, out ReadOnlySpan<char> absolutUri)
    {
        address = null;
        absolutUri = default;
...
public static class RelativeActorPath
{
    ...
    public static IEnumerable<string>? Unapply(string addr)
    {
        try
        {
            var isRelative = Uri.TryCreate(addr, UriKind.Relative, out _);

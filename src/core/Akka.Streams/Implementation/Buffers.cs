...
namespace Akka.Streams.Implementation
{
    ...
    internal static class Buffer
    {
        ...
        public static IBuffer<T> Create<T>(int size, int max)
        {
            ...
            {
                return new BoundedReferenceBuffer<T>(size);
            }
        }
    }
    ...
    internal static class FixedSizeBuffer 
    {
        ...
        public static FixedSizeBuffer<T> Create<T>(int size)
        {
            if (typeof(T).IsValueType)
            {
                ...
                if (((size - 1) & size) == 0)
                    return new PowerOfTwoFixedSizeValueTypeBuffer<T>(size);
                return new ModuloFixedSizeValueTypeBuffer<T>(size);
            }
            else
            {
                if (size < 1)
                    throw new ArgumentException("buffer size must be positive");
                if (((size - 1) & size) == 0)
                    return new PowerOfTwoFixedSizeReferenceBuffer<T>(size);
                return new ModuloFixedSizeReferenceBuffer<T>(size);
            }
        }
    }
    ...
    internal abstract class FixedSizeBuffer<T> : IBuffer<T>
    {
        ...
        public abstract void DropTail();
    }
    /// <summary>
    /// TBD
    /// </summary>
    /// <typeparam name="T">TBD</typeparam>
    internal abstract class FixedSizeValueTypeBuffer<T> : FixedSizeBuffer<T>
    {
        /// <summary>
        /// TBD
        /// </summary>
        ...
        protected FixedSizeValueTypeBuffer(int capacity)
        {
            Capacity = capacity;
        ...
        /// TBD
        /// </summary>
        public sealed override int Capacity { get; }
        /// <summary>
        /// TBD
        /// </summary>
        public sealed override int Used => (int)(WriteIndex - ReadIndex);
        /// <summary>
        /// TBD
        /// </summary>
        public sealed override bool IsFull => Used == Capacity;
        /// <summary>
        /// TBD
        /// </summary>
        public sealed override bool IsEmpty => Used == 0;
        /// <summary>
        /// TBD
        /// </summary>
        public sealed override bool NonEmpty => Used != 0;
        ...
        /// </summary>
        /// <returns>TBD</returns>
        public override T Peek() => Get(ReadIndex);
    ...
    /// </summary>
    /// <typeparam name="T">TBD</typeparam>
    internal class ModuloFixedSizeValueTypeBuffer<T> : FixedSizeValueTypeBuffer<T>
    {
        /// <summary>
        /// TBD
        /// </summary>
        /// <param name="size">TBD</param>
        public ModuloFixedSizeValueTypeBuffer(int size) : base(size)
        {
        }
    ...
    /// </summary>
    /// <typeparam name="T">TBD</typeparam>
    internal class PowerOfTwoFixedSizeValueTypeBuffer<T> : FixedSizeValueTypeBuffer<T> 
    {
        ...
        /// </summary>
        /// <param name="size">TBD</param>
        public PowerOfTwoFixedSizeValueTypeBuffer(int size) : base(size)
        {
            _mask = Capacity - 1;
        }
    ...
    /// </summary>
    /// <typeparam name="T">TBD</typeparam>
    internal abstract class FixedSizeReferenceBuffer<T> : FixedSizeBuffer<T>
    {
        /// <summary>
        /// TBD
        /// </summary>
        ...
        private readonly Box<T>[] _buffer;

        /// <summary>
        /// TBD
        /// </summary>
        /// <param name="capacity">TBD</param>
        protected FixedSizeReferenceBuffer(int capacity)
        {
            Capacity = capacity;
            _buffer = new Box<T>[capacity];
        }
        ...
        /// </summary>
        /// <param name="element">TBD</param>
        public sealed override void Enqueue(T element)
        {
            Put(WriteIndex, element, false);
            WriteIndex++;
        }
        ...
        /// <param name="maintenance">TBD</param>
        [MethodImpl(MethodImplOptions.NoInlining)]
        public void Put(long index, T element, bool maintenance) => _buffer[ToOffset(index, maintenance)] = new Box<T>(element);
        ...
        /// <param name="index">TBD</param>
        /// <returns>TBD</returns>
        public T Get(long index) => _buffer[ToOffset(index, false)].Item;

        /// <summary>
        /// TBD
        /// </summary>
        /// <returns>TBD</returns>
        public sealed override T Peek() => Get(ReadIndex);
        ...
        /// TBD
        /// </summary>
        public sealed override void DropHead()
        {
            // this is the only place where readIdx is advanced, so give ModuloFixedSizeBuffer
            // a chance to prevent its fatal wrap-around
            Put(ReadIndex, default(T), true);
        ...
        /// TBD
        /// </summary>
        public sealed override void DropTail()
        {
            WriteIndex--;
            Put(WriteIndex, default(T), false);
        }
    ...
    /// </summary>
    /// <typeparam name="T">TBD</typeparam>
    internal class ModuloFixedSizeReferenceBuffer<T> : FixedSizeReferenceBuffer<T>
    {
        /// <summary>
        /// TBD
        /// </summary>
        /// <param name="size">TBD</param>
        public ModuloFixedSizeReferenceBuffer(int size) : base(size)
        {
        }
    ...
    /// </summary>
    /// <typeparam name="T">TBD</typeparam>
    internal class PowerOfTwoFixedSizeReferenceBuffer<T> : FixedSizeReferenceBuffer<T> 
    {
        private readonly int _mask;
        ...
        /// </summary>
        /// <param name="size">TBD</param>
        public PowerOfTwoFixedSizeReferenceBuffer(int size) : base(size)
        {
            _mask = Capacity - 1;
        }
    ...
    /// </summary>
    /// <typeparam name="T">TBD</typeparam>
    internal sealed class BoundedReferenceBuffer<T> : IBuffer<T>
    {
        #region internal classes
        ...
        private sealed class FixedQueue : IBuffer<T>
        {
            ...
            private readonly Box<T>[] _queue = new Box<T>[Size];
            private readonly BoundedReferenceBuffer<T> _boundedBuffer;
            private int _head;
            private int _tail;

            public FixedQueue(BoundedReferenceBuffer<T> boundedBuffer)
            {
                _boundedBuffer = boundedBuffer;
                Capacity = boundedBuffer.Capacity;
            }
            ...
            public void Enqueue(T element)
            {
                ...
                {
                    _queue[_tail & Mask] = new Box<T>(element);
                    _tail += 1;
                }
            }
            ...
            public T Dequeue()
            {
                var pos = _head & Mask;
                var ret = _queue[pos];
                _queue[pos] = default(Box<T>);
                _head += 1;
                return ret.Item;
            }

            public T Peek() => _tail == _head ? default(T) : _queue[_head & Mask].Item;
            ...
            {
                _tail -= 1;
                _queue[_tail & Mask] = default(Box<T>);
            }
        }
        ...
        /// </summary>
        /// <param name="capacity">TBD</param>
        public BoundedReferenceBuffer(int capacity)
        {
            Capacity = capacity;
            _q = new FixedQueue(this);
        }
    ...
    internal readonly struct Box<T>// where T : class
    {

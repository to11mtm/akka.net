...
namespace RemotePingPong
{
    [MessagePackObject(keyAsPropertyName:true)]
        ...
        /// and a 8 KB <see cref="byte"/> array.  Tests the heavy-allocation path
        ...
        public sealed class LargePayloadEnvelope
        {
            ...
            /// <summary>8 KB payload buffer. Pre-filled with deterministic bytes. 🗜️</summary>
    ...
    internal class Program
    {
        ...
        [
            ...
            // ── Large payload (int64 + Guid + 8 KB byte[]) ──────────────────
            // CopilotNotes: Tests heavy-allocation / memory-pressure path; each message
            // carries a 8 KB byte array through the full serializer + transport stack. 📦
        ...
        internal enum PayloadMode
        {
            ...
            /// and a 8 KB <see cref="byte"/> array.  Exercises the large-payload /
        ...
        private static string PayloadLabel(PayloadMode mode) => mode switch
        {
            ...
            PayloadMode.LargePayload     => "Large payload (int64 + Guid + 8 KB byte[])",
            _                            => "Primitive long",
        };
        ...
        private static TransportMode ParseTransportMode(string? arg)
        {
            return (arg ?? "").ToLowerInvariant() switch
            {
                "stream" or "streams" => TransportMode.Streams,
        ...
        {
            TransportMode.Streams              => "Akka.Streams TCP + Protobuf",
        ...
        // CopilotNotes: Pre-allocate the 8 KB buffer once so CreatePingPayload is cheap.
        ...
        private static readonly byte[] _largeBuffer = BuildLargeBuffer(8 * 1024);
        ...
        private static object CreatePingPayload(PayloadMode mode) => mode switch
        {
            ...
            // CopilotNotes: LargePayloadEnvelope reuses the pre-built 8 KB buffer so we're
            ...
            SerializerMode serializerMode = SerializerMode.Default)
        {
            ...
            {
                TransportMode.Streams => ConfigurationFactory.ParseString(@"
                    akka.remote.dot-netty.tcp {
                        transport-class = ""Akka.Remote.Transport.Streams.TcpStreamTransport, Akka.Remote""
                        stream-write-buffer-size = 65536
                    }").WithFallback(ConfigurationFactory.ParseString(@"akka.remote.dot-netty.tcp.hostname = """ + ipOrHostname + @"""")
                    .WithFallback(ConfigurationFactory.ParseString(@"akka.remote.dot-netty.tcp.port = " + port))),
